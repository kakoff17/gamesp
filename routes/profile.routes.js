...
// GET "/api/profile/favs" => Recibe la información de los juegos favoritos

router.get("/favs", isAuthenticated, async (req, res, next) => {
  try {
    const userFavGames = await User.findById(req.payload._id).populate(
      "favGame"
    );
    res.json(userFavGames.favGame);
  } catch (error) {
    next(error);
  }
});
